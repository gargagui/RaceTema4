...
```{r}
select(cran, r_arch:country) #incluyendo las columnas r_arch y country
```
...
top_counts <- cran %>%
  group_by(package) %>%
  summarise(download_count = n()) %>%
  arrange(desc(download_count)) %>%
  filter(row_number() <= quantile(row_number(), probs = 0.99))
